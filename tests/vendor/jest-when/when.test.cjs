...
describe('When', () => {
  ...
  beforeEach(() => {
    ...
    resetAllWhenMocks = require('jest-when').resetAllWhenMocks
    verifyAllWhenMocksCalled = require('jest-when').verifyAllWhenMocksCalled
    WhenMock = require('jest-when').WhenMock
  })
  ...
  describe('when', () => {
    ...
    itNode('should print a full stacktrace if verification check fails', () => {
      ...
      // We changed this single line in @exodus/test testcases to fix the expected error message
      // Here is the original test line:
    ...
    itNode('fails verification check if all mocks were not called with line numbers', () => {
      ...
      try {
        ...
        // We changed the file path in @exodus/test
        // Original:
        // const currentFilePathPattern = /src(?:\\|\/)when\.test\.js:\d{3}(.|\s)*/
        // New:
        const currentFilePathPattern = /tests(?:\\|\/)vendor(?:\\|\/)jest-when(?:\\|\/)when\.test\.cjs:\d{3}(.|\s)*/
